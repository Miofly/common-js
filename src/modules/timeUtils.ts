...
const dateShift = (time: number) => new Date(new Date().getTime() - time * DAY_TIME) // 控制时间在多少天前/后

const monthDayShift = (monthNum: number, dayNum: number) =>
	new Date(new Date(new Date().setMonth(new Date().getMonth() - monthNum)).setDate(dayNum)) // 控制月/日
...
const tomorrowTime = dateShift(-1) // 明天
...
const timeUtils = {
	...
	dateShift ({
		...
		if (monthNum != 'no') {
			...
			let newDate = noSign
				? (showYear ? timeDate.getFullYear() : '') +
				this.addZero(timeDate.getMonth() + 1) +
				this.addZero(timeDate.getDate())
				: (showYear ? timeDate.getFullYear() + sign : '') +
				this.addZero(timeDate.getMonth() + 1) +
				sign +
				this.addZero(timeDate.getDate())
			...
			if (china) {
				newDate = `${
					(showYear ? `${ timeDate.getFullYear() }年` : '') + (timeDate.getMonth() + 1)
				}月${ timeDate.getDate() }日`
				newTime = `${ tempTime.split(':')[0] }时${ tempTime.split(':')[1] }分${
					tempTime.split(':')[2]
				}秒`
		...
		let newDate = noSign
			? `${ showYear ? timeDate.getFullYear() : '' }${ this.addZero(
				timeDate.getMonth() + 1
			) }${ this.addZero(timeDate.getDate()) }`
			: (showYear ? timeDate.getFullYear() + sign : '') +
			this.addZero(timeDate.getMonth() + 1) +
	...
	monthDayShift (monthNum: any, dayNum: any) {
		return new Date(
			new Date(new Date().setMonth(new Date().getMonth() - monthNum)).setDate(dayNum)
		)
	},
	// 数字不满10补0
	addZero (str: number) {
